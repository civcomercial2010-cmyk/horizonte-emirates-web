...
| ID | Mejora | H | Estado |
|---|---|---|---|
| M18 | Plan editorial SEO (8–12 guías) | H09 | 🟡 | **Blog construido y en working tree** (`public/blog/`): infraestructura completa (`blog.css` + `blog.js` autocontenidos, misma estética que la home, CSP-safe) + hub `/blog/` con filtro por categorías + **16 artículos** + 3 guías pilar. Cada artículo con JSON-LD `BlogPosting`+`BreadcrumbList`+`FAQPage`, TOC, bio de autor, fuentes y CTA al funnel. Página `/sobre/equipo.html` **retirada** (2026-06-12): redirect 301 a `/`. Estrategia en `docs/SEO_ESTRATEGIA.md`. Fuente `.md` en `contenido-blog/`. **DESPLEGADO Y VIVO**; `/blog/` vía `public/_redirects` (rewrite 200). Pendiente: guías restantes + alta en Search Console |
...
- **M18 🟡 (gran avance)** — Ejecutado el plan editorial SEO sobre el stack actual (HTML estático + Cloudflare, sin migración). Creado:
  ...
  - Página `/sobre/equipo.html` retirada (2026-06-12); EEAT vía `legal.html` + bloque confianza en home.
