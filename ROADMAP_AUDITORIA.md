...
| M08 | Healthcheck del pipeline (alerta 0 leads/72h) | H04 | ⬜ |
| M09 | Reducir fragilidad de captura (anti-SPAM + fallback) | H04 | ⬜ |
| M10 | Unificar taxonomía de eventos GA4 | H19 | ⬜ | Incluir limpieza de eventos clave "fantasma" en GA4 (`purchase`, `qualify_lead`, `close_convert_lead`, `manual_event_SUBMIT_L`) que el sitio NO dispara |
| M11 | Limpiar código muerto `captureUTM()` | H14 | ⬜ |
...
| M20 | JSON-LD `ItemList`/`Residence` + breadcrumb en proyectos | H20 | ⬜ |
| M21 | Política robots de IA + resolver doble `User-agent: *` | H11 | ⬜ |
| M22 | Corregir enlazado interno (footer → proyectos; quitar doble "Zonas") | — | ⬜ |
...
| M30 | `404.html` + HSTS 2 años + revisar ACAO | H21 | ⬜ |
